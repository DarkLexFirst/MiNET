		...
		public override void OnTick(Entity[] entities)
		{
			...
			foreach (var player in players.OrderBy(p => KnownPosition.DistanceTo(p.KnownPosition)))
			{
				if (player.GameMode != GameMode.Spectator && !player.HealthManager.IsDead && bbox.Intersects(player.GetBoundingBox() + 1))
				{
					...
					if (player.PickUpItem(this) && player.Inventory.SetFirstEmptySlot(Item, true))
					{
						{
			...
			if (rnd.NextDouble() < 0.5)
				foreach (ItemEntity itemEntity in Level.Entities.Values.Where(e => e is ItemEntity && e.KnownPosition.DistanceTo(KnownPosition) < 0.45f).ToArray())
				{
